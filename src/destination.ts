...
export async function readDestination<T extends IDestinationConfiguration>(
...
export async function readSubaccountDestination<
  T extends IDestinationConfiguration
>(
  destinationName: string,
  authorizationHeader?: string,
  subscribedSubdomain?: string
  ...
  const jwtToken = /bearer /i.test(authorizationHeader || "")
    ? (authorizationHeader || "").replace(/bearer /i, "")
    : undefined;
  return getSubaccountDestination<T>(
    access_token,
    destinationName,
    getService(),
    jwtToken
...
export async function readSubaccountDestinations<
  T extends IDestinationConfiguration
>(
  authorizationHeader?: string,
  subscribedSubdomain?: string,
  ...
  const jwtToken = /bearer /i.test(authorizationHeader || "")
    ? (authorizationHeader || "").replace(/bearer /i, "")
    : undefined;
  return getSubaccountDestinations<T>(
    access_token,
    getService(),
    jwtToken,
    regex
  ...
  private getAuthTokens() {
    if (this.getAuthenthicationType() === "BasicAuthentication") {
      return [
        {
          type: "Basic",
          value: Buffer.from(this.username + ":" + this.password).toString(
            "base64"
          ),
        },
      ];
    ...
    if (
      this.getAuthenthicationType() === "OAuth2UserTokenExchange" &&
      this.token
    ) {
      return [
        {
...
export interface IHTTPDestinationConfiguration
  extends IDestinationConfiguration {
  URL: string;
  Authentication:
    | "NoAuthentication"
    | "BasicAuthentication"
...
export interface IMailDestinationConfiguration
  extends IDestinationConfiguration {
...
async function getDestination<T extends IDestinationConfiguration>(
  access_token: string,
  destinationName: string,
  ds: IDestinationService,
  jwtToken: string | undefined
): Promise<IDestinationData<T>> {
  ...
  try {
    const destinationPromise = fetchDestination<T>(
      access_token,
      destinationName,
      ds,
      jwtToken
...
async function getSubaccountDestinations<T extends IDestinationConfiguration>(
  access_token: string,
  ds: IDestinationService,
  jwtToken: string | undefined,
  regex?: string
  ...
  try {
    ...
    if (regex) {
      response.data = response.data.filter(
        (destination: IDestinationConfiguration) =>
          destination.Name.match(regex)
      );
    }
...
async function getSubaccountDestination<T extends IDestinationConfiguration>(
  access_token: string,
  destinationName: string,
  ds: IDestinationService,
  jwtToken: string | undefined
...
async function fetchDestination<T extends IDestinationConfiguration>(
  access_token: string,
  destinationName: string,
  ds: IDestinationService,
  jwtToken: string | undefined
  ...
  if (process.env.destinations) {
    var destinations: { destinations: IMockDestinationConfiguration[] } =
      JSON.parse(process.env.destinations);
    if (destinations && Array.isArray(destinations)) {
      ...
      if (destination) {
        return new MockDestination(
          destination,
          access_token
        ).getDestination() as unknown as IDestinationData<T>;
      }
    }
  }
  ...
  const destination: IDestinationData<T> = (
    await axios({
...
async function fetchDestinations<T extends IDestinationConfiguration>(
  access_token: string,
  ds: IDestinationService,
  jwtToken: string | undefined
) {
  ...
  const destinations: IDestinationData<T> = (
    await axios({
      url: `${ds.uri}/destination-configuration/v1/subaccountDestinations`,
      method: "GET",
      headers,
...
async function fetchToken(
  ...
  const tokenBaseUrl = subscribedSubdomain
    ...
    : ds.url;
  const token: tokenCache.IOauthToken = (
    await axios({
